...
use crate::{AppWindow, ProjectGlobal};
...
impl TabType for TabTypeProject {
    ...
    fn register_callbacks(&self, app: &AppWindow, ctx: &CallbackCtx) {
        ...
        // ── Initial state ─────────────────────────────────────────────────────
        app.global::<ProjectGlobal>().set_project_tree(build_tree(&session.borrow()));
        ...
        app.global::<ProjectGlobal>().on_project_tree_selected(|_object_id| {});

        app.global::<ProjectGlobal>().on_close_project({
            let session  = session.clone();
            let app_weak = app.as_weak();
            move || {
                let proj_id = app_weak.upgrade()
                    .map(|u| u.global::<ProjectGlobal>().get_selected_project_id())
                    .unwrap_or(-1);
                if proj_id >= 0 {
                    session.borrow_mut().projects.remove(&(proj_id as u32));
                    if let Some(ui) = app_weak.upgrade() {
                        ui.global::<ProjectGlobal>().set_selected_project_id(-1);
                        ui.global::<ProjectGlobal>().set_project_tree(build_tree(&session.borrow()));
